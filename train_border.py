...
from lib.dataloader import SaltTrainDatasetBoundary, SaltTrainDataset
...
def train_cli(
    ...
    train_dataset = SaltTrainDatasetBoundary(train_ids, train_images, train_masks, augs[train_augs])
    val_dataset = SaltTrainDatasetBoundary(val_ids, train_images, train_masks, augs[val_augs])
    ...
    for epoch in range(n_epochs):
        ...
        for batch in train_loader:
            ...
            for metric_name, metric in metrics.items():
                metrics_dict[f'train_{metric_name}'] += metric(y_pred[:, :1, :, :], mask[:, :1, :, :].to(device))
        ...
        for batch in val_loader:
            ...
            with torch.no_grad():
                ...
                for i in range(mask.shape[0]):
                    predicted_mask = y_pred[i, :1, :, :]
                    original_mask = mask[i, :1, :, :]
